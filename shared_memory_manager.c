...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
		...
		uint32 Nsize = ROUNDUP(sva + size, PAGE_SIZE);
		sva = ROUNDDOWN(sva, PAGE_SIZE);
		int CheckShared = get_share_object_ID(ownerID, shareName);
		if (CheckShared == E_SHARED_MEM_NOT_EXISTS)
		{
			struct Share *Allocated_Object;
			int SharedObject = allocate_share_object(&Allocated_Object);
			if (SharedObject != E_NO_SHARE)
			{
				uint32 count = 0;
				for (int v = sva; v < Nsize; v += PAGE_SIZE)
				{
					uint32 *isPageTable = NULL;
					get_page_table(myenv->env_page_directory, v, &isPageTable);
					if (isPageTable == NULL)
					{
						isPageTable = create_page_table(myenv->env_page_directory, v);
					}
					struct FrameInfo *newFrame;
					int returned = allocate_frame(&newFrame);
					int mapped = map_frame(myenv->env_page_directory, newFrame, v,PERM_WRITEABLE | PERM_USER);
					add_frame_to_storage(shares[SharedObject].framesStorage, newFrame,count);
					count++;
				}
				shares[SharedObject].ownerID = ownerID;
				strcpy(shares[SharedObject].name, shareName);
				shares[SharedObject].size = size;
				shares[SharedObject].references = 1;
				shares[SharedObject].isWritable = isWritable;
				return SharedObject;
			}
			else
			{
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	struct Env* myenv = curenv; //The calling environment
		...
		if(CheckShared!=E_SHARED_MEM_NOT_EXISTS)
		{
			...
			Shared_Size=Shared_Size+va;
			uint32 count =0;
			for(int i=va;i<Shared_Size;i+=PAGE_SIZE)
			{
				uint32 *isPageTable=NULL;
				get_page_table(myenv->env_page_directory,i,&isPageTable);
				uint32* STorage=shares[CheckShared].framesStorage;
				struct FrameInfo *ptr_frame_infoS=get_frame_from_storage(STorage,count);
				if(isPageTable==NULL)
				{
					isPageTable=create_page_table(myenv->env_page_directory,i);
				}
				if (shares[CheckShared].isWritable == 1)
				{
					int mappeeeed = map_frame(myenv->env_page_directory,
							ptr_frame_infoS, i, PERM_WRITEABLE | PERM_USER);
				}
				else if (shares[CheckShared].isWritable == 0)
				{
					int mappeeeed = map_frame(myenv->env_page_directory,
							ptr_frame_infoS, i, ~PERM_WRITEABLE & PERM_USER);
				}
