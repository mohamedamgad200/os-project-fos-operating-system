...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	struct Semaphore* semaphore;
			uint32 x =get_semaphore_object_ID(ownerEnvID,semaphoreName);
						if (x == E_SEMAPHORE_EXISTS)
						{
							return E_SEMAPHORE_EXISTS;
						}
						else
						{
							uint32 y =allocate_semaphore_object(&semaphore);

							if (y !=E_NO_SEMAPHORE)

							{
								semaphores[y].ownerID=ownerEnvID;
								semaphores[y].value=initialValue;
							   strcpy(semaphores[y].name,semaphoreName);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int semId = get_semaphore_object_ID(ownerEnvID, semaphoreName);
		...
		if(semaphores[semId].value<=0)
			{
			struct Env* envItem = dequeue(&semaphores[semId].env_queue);
				sched_insert_ready(envItem);
